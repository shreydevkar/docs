...
```
=Data!A1                  ← value of A1 on the Data sheet
=SUM(Data!A1:A10)         ← sum a range from another sheet
='Monthly Budget'!B5      ← quoted name for sheets with spaces
=Sheet1!B2 * 1.1          ← mix cross-sheet reads with arithmetic
```

Sheet-name matching is case-insensitive. A reference to a sheet that doesn't exist resolves to `#REF!` at evaluation time.
...
| `#PARSE_ERROR!` | The formula string couldn't be tokenized — usually a stray character, unmatched quote, or invalid token. | See [Troubleshooting](/troubleshooting/common-errors#cell-shows-parse-error) for the full list of causes. Cross-sheet refs (`=Sheet2!A1`) and comparison ops (`=A1>B1`) *are* supported now; you'll see `#REF!` rather than a parse error if the referenced sheet doesn't exist. |
| `#REF!` | A cross-sheet reference pointed at a sheet that doesn't exist or has been deleted. | Check the sheet name and its quoting (`='Monthly Budget'!A1` for names with spaces). Sheet-name match is case-insensitive but the sheet has to actually exist. |
